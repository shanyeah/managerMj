            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    if (bottom > 0 + mHeaderMarginBottom + mTimeTextHeight / 2 && left < right &&
                        ...
                        drawText(mEventRects.get(i).event.getName() + "\n金我我我多" , mEventRects.get(i).rectF, canvas, originalTop, originalLeft);
//                        drawText1(mEventRects.get(i).event.getName(), mEventRects.get(i).rectF, canvas, originalTop, originalLeft);
                    } else
    ...
    private void drawText1(String text, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        if (rect.right - rect.left - mEventPadding * 2 < 0) return;
        ...
        // Draw text
        canvas.save();
        canvas.translate(originalLeft + mEventPadding, originalTop + mEventPadding+50);
        textLayout.draw(canvas);
        canvas.restore();
    }
