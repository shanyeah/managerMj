...
public class HoopChartView extends View {
    ...
    //统计图圆的直径
    private float mChartRadius = 260;
    //标注圆环的直径
    private float mLabelRadius = 26;
    //圆的粗细
    private float mStrokeWidth = 60;
    //标注线的长度
    private float mLineStroke = 15;
    //文字大小
...
//        float flagX = xys[0] * consF * (mChartRadius + marginARC) / 2 + centerX +30;
//        float flagY = xys[1] * sinF * (mChartRadius + marginARC) / 2 + centerY;
        float flagX = centerX +185 +50;   //标注圆位置
        float flagY = centerY +62 - 74*index;
...
//        flagY = flagY + mLabelRadius / 2 + whs[1] - 3;
        flagX = centerX +190+50;           //文字位置
        flagX = xys[0] == -1 ? flagX - textW - whs[0] : flagX + whs[0];
        flagY = centerY +74 - 74*index;
...
//        width += mLineStroke;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!initDrawParam) {
            ...
            centerX = mChartRadius / 2+80;
            centerY = mChartRadius / 2;
        }
        //初始化画布位置
        canvas.translate(mWidth / 2 - mChartRadius / 2-135-80, mHeight / 2 - mChartRadius / 2);
        // 绘制虚圆
        cyclePaint.setColor(Color.parseColor("#E3F0F6"));
        canvas.drawCircle(centerX-80, centerY, mChartRadius / 2, cyclePaint);
        // 执行绘制
