 ...
 */
public class PraiseNumAdapter extends BaseAdapter {
    private Context context;
    public List<PraiseMode> list;
    private DisplayImageOptions mOption;
    public int typeNum = 1;

    public PraiseNumAdapter(Context context, List<PraiseMode> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.praise_num_item, null);
	  ...
	  convertView.setTag(holder);
        ...
        holder.tvName.setText(list.get(position).date);
        holder.tvMemberId.setText("星期" + list.get(position).chineseWeek);
        ...
        switch (typeNum){
            case 1:
                ...
                break;
            case 2:
                holder.tvPraiseCount.setText("点餐次数:"+list.get(position).count);
                break;
            case 3:
                holder.tvPraiseCount.setText("充值次数:"+list.get(position).count);
                break;
        }
